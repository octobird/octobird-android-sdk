...
public class AutoResizeTextView extends TextView {
     ...
     * @param after  - The length of the replacement modified text.
     */
    @Override
     ...
     *
     * @param listener - Listener to receive resize notifications.
     */
     ...
     *
     * @param add  - Added height.
     * @param mult - Multiplied height.
     */
    @Override
     ...
     *
     * @param maxTextSize - Max text size limit.
     */
     ...
     *
     * @param minTextSize - Min text size limit.
     */
    public void setMinTextSize(float minTextSize) {
        mMinTextSize = minTextSize * getContext().getResources().getDisplayMetrics().density;
        requestLayout();
        invalidate();
    }
     ...
     *
     * @param addEllipsis - Ellipsis flag.
     */
    ...
    public void setMaxLine(int count) {
        maxLine = count;
        // if(count==1) setSingleLine(true);
    }

    /**
     * Resize the text size with specified width and height
     *
     * @param width  - New width of text.
     * @param height - New height of text.
     */
    public void resizeText(int width, int height) {
        ...
        StaticLayout l = getTextHeight(text, textPaint, width, targetTextSize);
        ...
        while (((l.getLineCount() > maxLine) || (l.getHeight() > height))
                && targetTextSize > mMinTextSize) {
            targetTextSize = Math.max(targetTextSize - 1, mMinTextSize);
            // *getContext().getResources().getDisplayMetrics().density
            // textHeight = getTextHeight(text, textPaint, width,
            // targetTextSize);
            l = getTextHeight(text, textPaint, width, targetTextSize);
        }

        // If we had reached our minimum text size and still don't fit, append
        // an ellipsis
        // if(mAddEllipsis && targetTextSize == mMinTextSize && textHeight >
        // height) {
        if (mAddEllipsis
                && ((targetTextSize == mMinTextSize)
                || (l.getHeight() > height) || (l.getLineCount() > maxLine))) {
            ...
            if (lastLine < 0)
                lastLine = 0;
            lastLine = lastLine > maxLine - 1 ? maxLine - 1 : lastLine;
    ...
    private StaticLayout getTextHeight(CharSequence source, TextPaint paint,
        ...
        layout.draw(sTextResizeCanvas);
        return layout;// layout.getHeight();
    }
